...
# Importiamo i pesi per sapere il totale delle materie (16)
...
class LunaGuiApp(ctk.CTk):
    def __init__(self):
        ...
        # Widget Progresso (inizializzati a None)
        self.lbl_prog = None
        self.prog_bar = None
        ...
        # 1. Costruisci l'interfaccia di gioco
        self._setup_ui()
    ...
    def _clear_options(self):
        """Nasconde tutti i widget nell'area opzioni (pulsanti e barra)."""
        for b in self.btn_options.values(): b.pack_forget()
        self.btn_next.pack_forget()
        if self.lbl_prog: self.lbl_prog.pack_forget()
        if self.prog_bar: self.prog_bar.pack_forget()
    ...
    def show_summary_screen(self):
        ...
        # --- CALCOLO PROGRESSO ---
        passed_unique = set()
        for lesson in self.session_state.completed_lessons:
            if lesson.score >= 8:
                # Pulisce "Diritto: Accesso" -> "Diritto"
                clean = lesson.topic.split(":")[0].strip()
                passed_unique.add(clean)

        total = len(DEFAULT_WEIGHTS)
        count = len(passed_unique)
        ratio = count / total if total > 0 else 0

        # --- CREAZIONE BARRA PROGRESSO ---
        if self.lbl_prog is None:
            self.lbl_prog = ctk.CTkLabel(self.options_frame, text="", font=("Helvetica", 14, "bold"),
                                         text_color="#10b981")

        if self.prog_bar is None:
            self.prog_bar = ctk.CTkProgressBar(self.options_frame, height=20, progress_color="#10b981")

        self.lbl_prog.configure(text=f"Carriera: {count}/{total} Materie ({int(ratio * 100)}%)")
        self.prog_bar.set(ratio)

        # Mostra la barra
        self.lbl_prog.pack(fill="x", pady=(10, 5))
        self.prog_bar.pack(fill="x", padx=20, pady=(0, 20))
        ...
        if not lessons:
            ...
            summary = "📜 REGISTRO LEZIONI RECENTI:\n\n"
            # Mostra solo le ultime 10 per non intasare
    ...
    def start_new_block(self):
        stop()
        self.step = "lesson"
        self._clear_options()  # Nasconde la barra
    ...
    def _display_report(self, text):
        ...
        # Mostra barra progresso anche qui? No, meglio mandare al riepilogo
        self._clear_options()
        self.btn_next.configure(text="TORNA AL RIEPILOGO & PROGRESSI ➤", command=self.show_summary_screen)
        self.btn_next.pack(fill="x", pady=5)
    ...
    def _chat_thread(self, txt):
        ...
        # Fix compatibilità con engine nuovo (stage parameter)
